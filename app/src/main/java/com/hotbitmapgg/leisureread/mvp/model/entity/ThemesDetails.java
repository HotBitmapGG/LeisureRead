...
public class ThemesDetails {
  ...
  private ArrayList<EditorsInfo> editors;
  ...
  public ThemesDetails(JSONObject json) {
    ...
    this.editors = new ArrayList<EditorsInfo>();
    ...
    if (null != arrayEditors) {
      ...
      for (int i = 0; i < editorsLength; i++) {
        ...
        if (null != item) {
          this.editors.add(new EditorsInfo(item));
        }
      }
    } else {
  ...
  public ArrayList<EditorsInfo> getEditors() {

    return this.editors;
  }


  public void setEditors(ArrayList<EditorsInfo> editors) {
